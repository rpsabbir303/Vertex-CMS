...
import {
  accessSecurity,
  accessSecurityEditorial,
  accessSecurityKeyInfo,
  securityCta,
} from "@/lib/marketing/security/content";
import { AccessSecurityAbstract } from "./AccessSecurityAbstract";
import { AccessSecurityFlowVisual } from "./AccessSecurityFlowVisual";
import {
...
export function AccessSecurityEditorialPage() {
  const { t } = useMarketing();
  const pageCopy = t.security.pages["access-security"];
  const ed = accessSecurityEditorial;

  return (
    <SecurityEditorialFrame>
      <section className="border-b border-brand-line bg-white" aria-labelledby="as-hero-heading">
        <div className="grid min-w-0 grid-cols-1 lg:grid-cols-2 lg:[grid-template-columns:minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <div className="flex min-h-[280px] min-w-0 items-center justify-center overflow-hidden border-brand-line bg-brand-soft/50 p-6 sm:min-h-[320px] sm:p-10 lg:min-h-[480px] lg:border-l">
            <div className="h-full max-h-[360px] w-full max-w-full">
              <AccessSecurityAbstract className="max-h-full max-w-full" />
            </div>
          </div>
        </div>
      </section>

      <SecurityEditorialTopicRail current="access-security" />

      <section className="border-b border-brand-line bg-white px-5 py-14 sm:px-8 sm:py-16" aria-labelledby="as-key-info-heading">
        <h2
          id="as-key-info-heading"
        ...
        <div className="mx-auto mt-10 w-full min-w-0 max-w-4xl border border-brand-line sm:flex">
          {accessSecurityKeyInfo.map((item, index) => (
            <div
              key={item.label}
      ...
      <section className="border-b border-brand-line bg-white px-5 py-14 sm:px-8 sm:py-16" aria-labelledby="as-main-heading">
        <div className="min-w-0 border border-brand-line">
          <div className="min-w-0 border-b border-brand-line px-6 py-10 sm:px-8 sm:py-12">
            <SecurityEditorialEyebrow>{t.security.accessSecurity.eyebrow}</SecurityEditorialEyebrow>
            <h2
              id="as-main-heading"
            ...
            <p className="mt-4 max-w-full text-[15px] leading-[1.75] text-brand-muted">{accessSecurity.supporting}</p>
          </div>
          ...
          <div className="grid min-w-0 grid-cols-1 lg:grid-cols-2 lg:[grid-template-columns:minmax(0,1fr)_minmax(0,1fr)]">
            ...
            <div className="min-w-0 px-6 py-8 sm:px-8 sm:py-10">
              <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-muted">Access controls</p>
              <SecurityEditorialDivider />
              <ul className="m-0 list-none p-0">
                {ed.accessControls.map((control, index) => (
      ...
      <section className="border-b border-brand-line bg-white px-5 py-14 sm:px-8 sm:py-16" aria-label="Access model">
        <AccessSecurityFlowVisual />
      </section>

      <section className="border-b border-brand-line bg-white px-5 py-14 sm:px-8 sm:py-16" aria-label="Access security topics">
        <div className="min-w-0 border border-brand-line">
          <div className="grid min-w-0 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 lg:[grid-template-columns:repeat(3,minmax(0,1fr))]">
            {ed.supportingTopics.map((note, index) => (
              <article
                key={note.title}
      ...
      <SecurityEditorialExplore current="access-security" />
      <SecurityEditorialCTA abstract={AccessSecurityAbstract} />
    </SecurityEditorialFrame>
  );
}
