    ...
    <div className="relative mx-auto aspect-square w-full max-w-[420px]" aria-hidden="true">
        ...
        {SATELLITES.map((s) => {
          ...
          return (
            <g key={s.id}>
              ...
              />
              <circle cx={p.x} cy={p.y} r={hot ? 6 : 4} fill={hot ? "#3FE844" : "#2F7A3A"} className={hot ? "security-lime-dot" : undefined} />
            </g>
          );
        })}
      ...
      {SATELLITES.map((s) => {
        ...
        return (
          <span
            key={s.id}
            className={`absolute -translate-x-1/2 -translate-y-1/2 text-[10px] font-semibold uppercase tracking-[0.14em] ${
              hot ? "text-[#0D0D0D]" : "text-[#163326]/55"
            }`}
